 ...
 * Requires company name and domain.
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // API docs: https://docs.apollo.io/reference/create-an-account
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        return NextResponse.json<ApolloCreateResponse>(
          { success: false, error: "Apollo API requires a master API key to create accounts" },
          { status: 403 }
        );
      }
